    ...
    <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-hidden">
      ...
      <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex justify-between items-center">
          <div>
            <h2 class="text-lg font-semibold text-gray-900">Manage Team Members</h2>
      ...
      <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
        <div class="flex justify-between items-center">
          ...
          <div class="flex space-x-3">
            ...
            >
              <span v-if="loading">
                <svg class="w-4 h-4 mr-2 animate-spin" viewBox="0 0 24 24" fill="currentColor">
...
// ============= COMPLETE REPLACEMENT OF THE saveChanges METHOD =============
// In EmployeeAssignmentModal.vue, COMPLETELY replace the saveChanges method with this:
const saveChanges = async () => {
  ...
  try {
    ...
    for (const employee of employeesToAdd) {
      ...
      promises.push(
        fetch(`${API_BASE_URL}/api/v2/teams/${props.team.id}/employees/${employee.id}`, {
          method: 'POST',
    ...
    for (const employeeId of employeesToRemove) {
      ...
      promises.push(
        fetch(`${API_BASE_URL}/api/v2/teams/${props.team.id}/employees/${employeeId}`, {
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json' }
        }).then(async response => {
          if (!response.ok) {
