...
public final class PlanningUtils {
    ...
    public static String getDelaiForPlanning(Map<String, Object> order) {
        if (order == null) {
            log.warn(" Null order provided, using default delai '{}'", DEFAULT_DELAI);
    ...
    public static String validateDelai(String delai) {
        ...
        log.warn(" Invalid delai '{}', using default '{}'", delai, DEFAULT_DELAI);
        return DEFAULT_DELAI;
    }
    ...
    public static int validateStatus(Integer status) {
        ...
        log.warn(" Invalid status '{}', using default '{}'", status, DEFAULT_STATUS);
        return DEFAULT_STATUS;
    }
    ...
    public static int calculateDuration(int cardCount, int minutesPerCard) {
        if (cardCount < 0) {
            log.warn(" Negative card count {}, using 0", cardCount);
            cardCount = 0;
        }
        if (minutesPerCard < 1) {
            log.warn(" Invalid minutesPerCard {}, using default {}",
    ...
    public static int getCardCount(Map<String, Object> order) {
        ...
        log.warn(" No card count found in order, using 0");
        return 0;
    }
