...
public class ApiKeyAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (!request.getRequestURI().startsWith("/api/sync/")) {
            log.info(" Skipping API Key check - not a sync endpoint");
            filterChain.doFilter(request, response);
            return;
        }
        ...
        if (apiKey != null && apiKey.equals(apiSyncKey)) {
            ...
            log.warn(" Invalid or missing API Key for sync endpoint: {}", request.getRequestURI());
            log.warn(" API Key was null: {}", apiKey == null);
        }
