...
public class ImprovedPlanningService {
    ...
    private int assignScanningOrdersToEmployees(
        ...
        if (orders.isEmpty() || workloads.isEmpty()) {
            log.info(" Skipping scanning assignment: {} orders, {} employees",
        ...
        for (Map<String, Object> order : orders) {
            ...
            if (leastLoaded == null) {
                log.warn(" No scanner employee available for order {}", orderId);
                continue;
            }
    ...
    private int assignOrdersToEmployees(
        ...
        if (orders.isEmpty() || workloads.isEmpty()) {
            log.info(" Skipping assignment for status {}: {} orders, {} employees",
        ...
        for (Map<String, Object> order : orders) {
            ...
            if (cardCount == null || cardCount == 0) {
                log.warn(" Order {} has no cards, skipping", orderId);
                continue;
            }
            ...
            if (leastLoaded == null) {
                log.warn(" No employee available for order {}", orderId);
                continue;
            }
